...
def enviar_email_recuperacao(destinatario, codigo):
  ...
  porta = 587
  email_remetente = "seu_email@novomundo.com"  # Substitua pelo e-mail remetente
  senha_remetente = "sua_senha_de_app"  # Substitua pela senha de aplicativo do e-mail

  msg = MIMEMultipart()
  msg["From"] = email_remetente
  msg["To"] = destinatario
  msg["Subject"] = "Código de Recuperação de Senha - Sistema PM e RIO"

  corpo = (
      f"Olá,\n\nVocê errou sua senha 3 vezes. Seu código de recuperação de"
      f" 4 dígitos é: {codigo}\n\nUtilize este código na tela de login para"
...
<link rel="manifest" href="{{ url_for('static', filename='manifest.json') }}">
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        /* MODAL DE VÍDEO INTEGRADO AO APP */
        .modal-video-overlay {
            display: none;
            position: fixed;
        ...
        .error { background-color: #fff5f5; color: #c53030; padding: 10px; border-radius: 6px; font-size: 13px; margin-bottom: 15px; border: 1px solid #feb2b2; }
...
def login():
  erro = None
  recuperacao_ativa = session.get("recuperacao_ativa", False)

  if request.method == "POST":
    input_email = request.form.get("email", "").strip().lower()

    try:
      # Mantém a sua conexão e leitura atual da planilha
      planilha = conectar_google_sheets()  # Utilize sua função existente
      aba_usuarios = planilha.worksheet("Usuarios")
      usuarios = aba_usuarios.get_all_records()

      # Se o usuário enviou o código de 4 dígitos e a nova senha
      if "codigo_digitado" in request.form:
        codigo_informado = request.form.get("codigo_digitado").strip()
        nova_senha = request.form.get("nova_senha")

        if codigo_informado == str(session.get("codigo_aleatorio")):
          cell = aba_usuarios.find(input_email)
          if cell:
            # Atualiza a coluna da senha (ajuste o número da coluna se necessário, ex: 3)
            aba_usuarios.update_cell(cell.row, 3, nova_senha)

          session.pop("recuperacao_ativa", None)
          session.pop("codigo_aleatorio", None)
          session.pop("tentativas_erro", None)
          session.pop("email_recuperacao", None)
          erro = "Senha alterada com sucesso! Faça login com a nova senha."
        else:
          erro = "Código de verificação incorreto."
          recuperacao_ativa = True

        return render_template_string(
            TEMPLATE_HTML, erro=erro, recuperacao_ativa=recuperacao_ativa
        )

      # Fluxo normal de validação de login
      input_senha = request.form.get("senha")
      usuario_encontrado = None

      for idx, u in enumerate(usuarios, start=2):
      ...
      if (
          usuario_encontrado
          and str(usuario_encontrado.get("SENHA", "")) == input_senha
      ):
        # Sucesso: limpa os contadores de erro
        session.pop("tentativas_erro", None)
        session.pop("recuperacao_ativa", None)
        session.pop("codigo_aleatorio", None)
        ...
        # Errou a senha: incrementa o contador
        tentativas = session.get("tentativas_erro", 0) + 1
        session["tentativas_erro"] = tentativas

        if tentativas >= 3:
          codigo_4_digitos = str(random.randint(1000, 9999))
          session["codigo_aleatorio"] = codigo_4_digitos
          session["recuperacao_ativa"] = True
          session["email_recuperacao"] = input_email

          enviar_email_recuperacao(input_email, codigo_4_digitos)
          erro = (
              "Você excedeu 3 tentativas incorretas. Um código de"
              " recuperação de 4 dígitos foi enviado para o seu e-mail."
          )
          recuperacao_ativa = True
        else:
          restantes = 3 - tentativas
          erro = (
              f"E-mail ou Senha incorretos. Você tem mais {restantes}"
    ...
    try:
      ...
      if produto_selecionado:
        ...
        for item in produtos_rio:
          if str(item.get("PRODUTO", "")) == produto_selecionado:
            item_escolhido = item
            break
        ...
        if item_escolhido:
          ...
          valor = formatar_moeda(
              item_escolhido.get("VALOR R$", ""), manter_todos_decimais=False
          )
          video = (
          ...
          def destacar_termos(texto):
            ...
            texto_formatado = re.sub(
                r"(Foco:)", r"<b>\1</b>", texto, flags=re.IGNORECASE
            )
            texto_formatado = re.sub(
                r"(Descrição:)", r"<b>\1</b>", texto_formatado, flags=re.IGNORECASE
            )
            texto_formatado = re.sub(
                r"(Funcionalidades:)",
                r"<b>\1</b>",
            ...
            texto_formatado = re.sub(
                r"(Diferencial Estratégico:|Diferencial Estrategico:)",
                r"<b>\1</b>",
                texto_formatado,
                flags=re.IGNORECASE,
            )
          ...
          contato_texto = (
              f"{nome_vendedor}, Torre de Controle da Novo Mundo Caminhões -"
              " 📞 (81) 99686-0674"
          )
...
👤 *Contato:* {contato_texto}"""
          ...
          link_wpp_compartilhar = (
              "https://api.whatsapp.com/send?text="
              + urllib.parse.quote(texto_whatsapp)
          )

          url_video_embed = converter_para_embed(video)

          btn_ver_video = (
              f'<button type="button" class="btn-acao btn-video"'
              f' onclick="abrirVideoModal(\'{url_video_embed}\')">▶ Assistir'
              " Vídeo</button>"
              if video
              else ""
          )
          btn_enviar_wpp = (
              f'<a href="{link_wpp_compartilhar}" target="_blank"'
              ' rel="noopener noreferrer" class="btn-acao btn-whatsapp">📤 Enviar'
              " WhatsApp</a>"
          )
          ...
          conteudo = """
                    <div>
                        <a href="/modulo/rio" class="btn-voltar">‹ Voltar</a>
                        <p style="color: #c53030;">Produto não encontrado.</p>
                    </div>
                    """
      else:
        botoes_produtos = ""
        for item in produtos_rio:
          ...
          if prod:
            botoes_produtos += f'<a href="/modulo/rio?produto={prod}" class="submenu-btn">{prod}</a>'
      ...
      conteudo = f"""
            <div style="color: #c53030; background: #fff5f5; padding: 15px; border-radius: 8px; border: 1px solid #feb2b2;">
                <b>Erro ao carregar os dados da aba RIO:</b> {e}
            </div>
            """
...
👤 *Contato:* {contato_texto}"""
          ...
          conteudo = """
                    <div>
                        <a href="/modulo/pm" class="btn-voltar">‹ Voltar</a>
                        <p style="color: #c53030;">Plano não encontrado.</p>
                    </div>
                    """
      else:
        botoes_produtos = ""
        for item in produtos_pm:
          prod = item.get("PRODUTO", "")
          if prod:
            botoes_produtos += f'<a href="/modulo/pm?produto={prod}" class="submenu-btn">{prod}</a>'
      ...
      conteudo = f"""
            <div style="color: #c53030; background: #fff5f5; padding: 15px; border-radius: 8px; border: 1px solid #feb2b2;">
                <b>Erro ao carregar os dados da aba PM:</b> {e}
            </div>
            """
    ...
    try:
      ...
      if produto_selecionado:
        ...
        for item in dados_precos:
          ...
          if str(nome_item) == produto_selecionado:
            item_escolhido = item
            break
        ...
        if item_escolhido:
          ...
          if valor_ficha_bruto:
            texto_wpp_ficha = (
                f"📄 *Ficha Técnica - {titulo_principal}*\n\nConsulte o"
                f" documento completo no link abaixo:\n{link_ficha_tecnica}"
            )
          ...
          planos_km_info = [
              {
                  ...
                  "km_col": (
                      "PREV_VALOR KM"
                      if "PREV_VALOR KM" in item_escolhido
                      else "KM"
                  ),
                  "mensal_col": (
                      "VALOR MENSAL"
                      if "VALOR MENSAL" in item_escolhido
                      else ""
                  ),
                  "total_col": (
                      "TOTAL CONTRATO"
                      if "TOTAL CONTRATO" in item_escolhido
                      else ""
                  ),
              },
              {
                  ...
                  "km_col": (
                      "MAX_VALOR KM"
                      if "MAX_VALOR KM" in item_escolhido
                      else "KM_1"
                  ),
                  "mensal_col": (
                      "VALOR MENSAL_1"
                      if "VALOR MENSAL_1" in item_escolhido
                      else ""
                  ),
                  "total_col": (
                      "TOTAL CONTRATO_1"
                      if "TOTAL CONTRATO_1" in item_escolhido
                      else ""
                  ),
              },
              {
                  ...
                  "km_col": (
                      "PLUS_VALOR KM"
                      if "PLUS_VALOR KM" in item_escolhido
                      else "KM_2"
                  ),
                  "mensal_col": (
                      "VALOR MENSAL_2"
                      if "VALOR MENSAL_2" in item_escolhido
                      else ""
                  ),
                  "total_col": (
                      "TOTAL CONTRATO_2"
                      if "TOTAL CONTRATO_2" in item_escolhido
                      else ""
                  ),
              },
          ]
          ...
          for p in planos_km_info:
            km_val = formatar_moeda(
                item_escolhido.get(p["km_col"], ""), manter_todos_decimais=True
            )
            mensal_val = (
                formatar_moeda(
                    item_escolhido.get(p["mensal_col"], ""),
                    manter_todos_decimais=False,
                )
                if p["mensal_col"]
                else ""
            )
            total_val = (
                formatar_moeda(
                    item_escolhido.get(p["total_col"], ""),
                    manter_todos_decimais=False,
                )
                if p["total_col"]
                else ""
          ...
          planos_hora_info = [
              {
                  ...
                  "hora_col": (
                      "PREV_VALOR HORA"
                      if "PREV_VALOR HORA" in item_escolhido
                      else "HORA"
                  ),
              },
              {
                  ...
                  "hora_col": (
                      "MAX_VALOR HORA"
                      if "MAX_VALOR HORA" in item_escolhido
                      else "HORA_1"
                  ),
              },
              {
                  ...
                  "hora_col": (
                      "PLUS_VALOR HORA"
                      if "PLUS_VALOR HORA" in item_escolhido
                      else "HORA_2"
                  ),
              },
          ]
          ...
          if hora_geral_val:
            for p in planos_hora_info:
              ...
              if p["nome"] == "Plano PREV":
                chaves_mensal = [
                    k for k in item_escolhido.keys() if k.startswith("VALOR MENSAL")
                ]
                if len(chaves_mensal) > 3:
                  mensal_val_crua = item_escolhido.get(chaves_mensal[3], "")
                chaves_total = [
                    k for k in item_escolhido.keys() if k.startswith("TOTAL CONTRATO")
                ]
                if len(chaves_total) > 3:
                  total_val_crua = item_escolhido.get(chaves_total[3], "")
              elif p["nome"] == "Plano MAX":
                chaves_mensal = [
                    k for k in item_escolhido.keys() if k.startswith("VALOR MENSAL")
                ]
                if len(chaves_mensal) > 4:
                  mensal_val_crua = item_escolhido.get(chaves_mensal[4], "")
                chaves_total = [
                    k for k in item_escolhido.keys() if k.startswith("TOTAL CONTRATO")
                ]
                if len(chaves_total) > 4:
                  total_val_crua = item_escolhido.get(chaves_total[4], "")
              elif p["nome"] == "Plano PLUS":
                chaves_mensal = [
                    k for k in item_escolhido.keys() if k.startswith("VALOR MENSAL")
                ]
                if len(chaves_mensal) > 5:
                  mensal_val_crua = item_escolhido.get(chaves_mensal[5], "")
                chaves_total = [
                    k for k in item_escolhido.keys() if k.startswith("TOTAL CONTRATO")
                ]
                if len(chaves_total) > 5:
                  total_val_crua = item_escolhido.get(chaves_total[5], "")

              hora_val = formatar_moeda(
                  hora_val_crua, manter_todos_decimais=True
              )
              mensal_val = formatar_moeda(
                  mensal_val_crua, manter_todos_decimais=False
              )
              total_val = formatar_moeda(
                  total_val_crua, manter_todos_decimais=False
              )
          ...
          conteudo = """
                    <div>
                        <a href="/modulo/valores" class="btn-voltar">‹ Voltar</a>
                        <p style="color: #c53030;">Item não encontrado.</p>
                    </div>
                    """
      else:
        botoes_itens = ""
        for item in dados_precos:
          ...
          if nome_item:
            botoes_itens += f'<a href="/modulo/valores?produto={nome_item}" class="submenu-btn">{nome_item}</a>'
      ...
      conteudo = f"""
            <div style="color: #c53030; background: #fff5f5; padding: 15px; border-radius: 8px; border: 1px solid #feb2b2;">
                <b>Erro ao carregar os dados da aba PM_Precos:</b> {e}
            </div>
            """
    ...
    try:
      ...
      if informe_selecionado:
        ...
        for item in dados_informes:
          ...
          if str(assunto) == informe_selecionado:
            item_escolhido = item
            break
        ...
        if item_escolhido:
          ...
          conteudo = """
                    <div>
                        <a href="/modulo/informes" class="btn-voltar">‹ Voltar</a>
                        <p style="color: #c53030;">Informe não encontrado.</p>
                    </div>
                    """
      else:
        botoes_informes = ""
        for item in dados_informes:
          assunto = item.get("ASSUNTO", "")
          if assunto:
            botoes_informes += f'<a href="/modulo/informes?item={assunto}" class="submenu-btn">{assunto}</a>'
      ...
      conteudo = f"""
            <div style="color: #c53030; background: #fff5f5; padding: 15px; border-radius: 8px; border: 1px solid #feb2b2;">
                <b>Erro ao carregar os dados da aba Informes:</b> {e}
            </div>
            """
    ...
    try:
      ...
      if argumento_selecionado:
        ...
        for item in dados_argumentos:
          pergunta = item.get("QUESTIONAMENTO", "")
          if str(pergunta) == argumento_selecionado:
            item_escolhido = item
            break
...
👤 *Contato:* {contato_texto}"""
          ...
          conteudo = """
                    <div>
                        <a href="/modulo/argumentos" class="btn-voltar">‹ Voltar</a>
                        <p style="color: #c53030;">Argumento não encontrado.</p>
                    </div>
                    """
      else:
        botoes_argumentos = ""
        for item in dados_argumentos:
          pergunta = item.get("QUESTIONAMENTO", "")
          if pergunta:
            botoes_argumentos += f'<a href="/modulo/argumentos?item={pergunta}" class="submenu-btn">{pergunta}</a>'
      ...
      conteudo = f"""
            <div style="color: #c53030; background: #fff5f5; padding: 15px; border-radius: 8px; border: 1px solid #feb2b2;">
                <b>Erro ao carregar os dados da aba Argumentos:</b> {e}
            </div>
            """
